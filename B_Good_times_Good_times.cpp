...
/*
 * OBS:
 * - x is good => x's digit string has <=2 distinct chars.
 * - L = len(digits of x). Pick y = 10^L + 1 (digits {1,0} or {1} if L=0 -> always good).
 * - x*y = x*10^L + x = concat(str(x), str(x)) since x < 10^L (no carry across blocks).
 * - Concatenation of x with itself keeps the exact same digit set => x*y is good too.
 * - y <= 10^8+1 < 10^9 since x < 10^8 (L<=8), and y>=11>=2. Bounds satisfied for any good x.
 *
 * APR: pad x with itself via 10^L+1 multiplier, no digit casework needed.
 */
void solve(){
   ll x;cin>>x;
