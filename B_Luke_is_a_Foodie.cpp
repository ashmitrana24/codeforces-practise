...
Timestamp: 2026-06-26 21:37:02 IST
*/
#include <bits/stdc++.h>
using namespace std;
/* clang-format off */
...
/*
 * OBS:
 *   we are given a inequality and we can open it |y|<=x ===> -x<=y<=x
 *   we have to eat all piles of food , its compulsory
 *   
 * APR:
 *      so we find the r of each element and we have to change the 'V' whenever there is no intersection b/w the rs
 */
void solve(){
    ll n,x;cin>>n>>x;
    vll a(n);f(i,0,n)cin>>a[i];
    v<pll>r(n);
    f(i,0,n)r[i]={a[i]-x,a[i]+x};
    ll cnt=0;
    f(i,1,n){
        if(r[i].ff>r[i-1].ss||r[i].ss<r[i-1].ff) cnt++;
        else r[i]={max(r[i].ff,r[i-1].ff),min(r[i].ss,r[i-1].ss)};
    }
    cout<<cnt<<endl;
}
int main(){
