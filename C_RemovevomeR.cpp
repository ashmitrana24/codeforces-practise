...
/*
 * OBS:
 * - Brute-force BFS over reachable strings (verified for all binary strings, len<=8)
 *   shows the answer depends only on the number of "runs" (maximal equal-char blocks).
 * - runs==2 (string is a single block of one char followed by a single block of the
 *   other, e.g. 111000) => answer is always 2, you can never erase the lone boundary.
 * - runs==1 (monochromatic) or runs>=3 => answer is always 1.
 *
 * APR: just count runs, O(n) per test case, no simulation needed.
 */
void solve(){
   int n;cin>>n;
   string s;cin>>s;
   int runs=1;
   f(i,1,n) if(s[i]!=s[i-1]) runs++;
